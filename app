...
def dateCrawling(mv_title):
    ...
    mv_date = soup.find('dd', {'class': 'frst'})
...
def post():
    ...
    if(moviedata == 0): #네이버 영화 크롤링 결과가 없을 경우 "결과 없음" 출력
        ...
        return render_template(
            ...
            mv_actor = mv_actor,
            mv_poster=mv_poster,
            mv_link = "https://tv.naver.com/search/clip?query=" + mv_title + "&sort=rel&isTag=false",
...
def star1():
    print(starnum)
    return render_template('star.html', title="star", starnum = starnum)
