...
def calling(file):
    ...
    for sentence1 in sentences_tag:
        ...
        for word, tag in sentence1:
            if tag in ['Adjective'] and ("이다" not in word) and ("아니다" not in word) and ("있다" not in word) and (
                    "없다" not in word) and ("많다" not in word) and ("같다" not in word) and ("그렇다" not in word) and (
   ...
   # print("별점(소수점둘째자리까지) : ", "%0.2f" % starnum)
    ...
    # 7. 워드클라우드 만들기
    wordcloud = WordCloud(font_path='c:/Windows/Fonts/malgun.ttf', background_color='white', width=1200,
